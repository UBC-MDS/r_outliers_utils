...
test_that('Plot should use geom_violin in default and map value to x-axis, and variables to y-axis, also facet.', {
  ...
  expect_true("name"  == rlang::get_expr(visualize_outliers(df, columns='SepalLengthCm')$mapping$x))
  expect_true("value" == rlang::get_expr(visualize_outliers(df, columns='SepalLengthCm')$mapping$y))
